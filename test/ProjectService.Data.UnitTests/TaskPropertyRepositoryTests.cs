...
namespace LT.DigitalOffice.ProjectService.Data.UnitTests
{
  public class TaskPropertyRepositoryTests
  {
    ...
    public void ShouldExist()
    {
      ...
      Assert.IsTrue(_repository.AreExist(_taskPropertyId, (TaskPropertyType)_type));
    }

    [Test]
    public void ShouldNotExist()
    {
      Assert.IsFalse(_repository.AreExist(Guid.NewGuid()));
      Assert.IsFalse(_repository.AreExist(Guid.NewGuid(), (TaskPropertyType)_type));
      Assert.IsFalse(_repository.AreExist(_taskPropertyId, TaskPropertyType.Priority));
    }

    #endregion

    [Test]
    public void ShouldGetPropertySuccessfully()
    {
    ...
    [Test]
    public void ShouldCreateTaskPropertySuccessfully()
    {
    ...
    public void ShouldDbTaskPropertiesReturnByNameSuccessful()
    {
        var filter = new FindTaskPropertiesFilter
        {
            Name = "Name",
            SkipCount = 0,
            TakeCount = _dbTaskProperties.Count
        };

        SerializerAssert.AreEqual(_dbTaskProperties, _repository.Find(filter, out int totalCount));
        Assert.AreEqual(_dbTaskProperties.Count, totalCount);
    }

    [Test]
    public void ShouldDbTaskPropertiesReturnByShortNameSuccessfully()
    {
        var filter = new FindTaskPropertiesFilter
        {
          Name = "Nam",
    ...
    [Test]
    public void ShouldDbTaskPropertiesReturnByProjectIdSuccessfully()
    {
        var filter = new FindTaskPropertiesFilter
        {
        ...
        SerializerAssert.AreEqual(expectedResult, _repository.Find(filter, out int totalCount));
        Assert.AreEqual(expectedResult.Count, totalCount);
    }

    [Test]
    public void ShouldDbTaskPropertiesReturnByAuthorIdSuccessfully()
    {
      var filter = new FindTaskPropertiesFilter
      {
        AuthorId = _dbTaskProperties[0].CreatedBy,
        SkipCount = 0,
        TakeCount = _dbTaskProperties.Count
      };
    ...
    [Test]
    public void ShouldDbTaskPropertiesReturnByTypeSuccessfully()
    {
      var filter = new FindTaskPropertiesFilter
      {
        Type = (TaskPropertyType)_type,
        SkipCount = 0,
        TakeCount = _dbTaskProperties.Count
      };
